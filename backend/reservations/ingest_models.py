...
class RoomPlacementListIngest(BaseModel):
    ...
    used downstream:
        ...
        ticket_id_in_secret_party
    """
    placement_verified: bool = Field(alias='Placement Verified')
    floor: int = Field(alias='Floor')
    room: str = Field(alias='Room')
    room_type: str = Field(alias='Room Type')
    room_features: Optional[str] = Field(alias='"Room Features (Accesbility, Lakeview, Smoking)(not visible externally)"')
    ...
    art_room: Optional[bool] = Field(alias='Art Room')
    ...
    changeable: Optional[bool] = Field(alias='Changeable')
    ...
    paying_guest: Optional[bool] = Field(alias='Paying guest?')
    ...
    class Config:
        allow_population_by_field_name = True  # allows data to be populated in the model by field names, not just aliases
